...
GreedyStratifiedTeams <- function(group_id, group_score,
  ...
  while(length(groups_to_assign)) {
    ...
    min_team <- .get_strata_min_team(team_scores, max_num_team)
...
.get_strata_min_team <- function(df, max_num_team) {
  ## For R CMD check
  team_id <- med_score <- num_players <- score <-  NULL

  team_min <- dplyr::group_by(df, team_id) %>%
    dplyr::summarize(mean_strata_score = sum(strata_score) / sum(num_strata),
                     mean_score = sum(score) / (sum(num_players) - sum(num_strata)),
                     num_strata = sum(num_strata),
                     num_players = sum(num_players)) %>%
    dplyr::ungroup() %>%
    dplyr::arrange(num_strata,mean_strata_score, mean_score, num_players) %>%
